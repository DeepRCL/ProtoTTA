"""
Run Test-Time Adaptation (TTA) Inference on Amazon-C (Corrupted Amazon) Dataset.

Evaluates ProtoLens model with various TTA methods on corrupted Amazon data.
Supports more severe corruption levels (70%, 80%, 90%) for meaningful TTA testing.

Usage:
    python run_inference_amazon_c.py --corruption_type aggressive --severity 90
    python run_inference_amazon_c.py --corruption_type mixed --severity 80 --methods baseline tent prototta

Output:
    - Console: Accuracy, classification report, confusion matrix
    - Datasets/Amazon-C/results/: Detailed results JSON
"""
...
# ============================================================================
# Configuration (matches run_inference_yelp_c.py)
# ============================================================================
...
class Cfg:
    ...
    class Optim:
        def __init__(self):
            self.METHOD = 'Adam'
            self.LR = 0.00001  # Low LR to prevent collapse
            self.BETA = 0.9
            self.WD = 0.0
            self.STEPS = 1
...
def parse_args():
    parser = argparse.ArgumentParser(description='TTA evaluation on Amazon-C')
    
    # Dataset settings
    parser.add_argument('--data_dir', type=str, default='Datasets/Amazon-C',
                       help='Directory containing Amazon-C datasets')
    parser.add_argument('--corruption_type', type=str, default='aggressive',
                       choices=['qwerty', 'swap', 'remove_char', 'mixed', 'aggressive', 'clean'],
                       help='Type of corruption')
    parser.add_argument('--severity', type=int, default=70,
    ...
    parser.add_argument('--geo_filter', action='store_true', default=True,
                       help='Use geometric filtering for ProtoTTA (enabled by default)')
    parser.add_argument('--no_geo_filter', action='store_true', default=False,
                       help='Explicitly disable geometric filtering')
    parser.add_argument('--geo_threshold', type=float, default=0.1,
                       help='Geometric filter threshold (only used if --geo_filter is set)')
    parser.add_argument('--sigmoid_temperature', type=float, default=5.0,
                       help='Sigmoid temperature for prototype similarities (higher=wider spread, default=5.0)')
    ...
    parser.add_argument('--e_margin', type=float, default=0.6,
                       help='EATA entropy margin (higher=adapt more samples, max=0.693 for binary)')
    parser.add_argument('--d_margin', type=float, default=0.05,
                       help='EATA diversity margin (anti-forgetting)')
...
def load_corrupted_data(data_dir: str, corruption_type: str, severity: int) -> pd.DataFrame:
    ...
    if not os.path.exists(filepath):
        raise FileNotFoundError(f"Dataset not found: {filepath}\nRun generate_amazon_c.py first!")
...
def print_results(results: Dict, corruption_type: str, severity: int):
    ...
    print("\n" + "=" * 80)
    print(f"RESULTS: Amazon-C ({corruption_type}, severity={severity}%)")
    print("=" * 80)
...
def main():
    ...
    print("\n" + "=" * 80)
    print("ProtoLens TTA Evaluation on Amazon-C")
    print("=" * 80)
    ...
    if 'prototta' in args.methods:
        ...
        print(f"ProtoTTA: geo_filter={use_geo}, threshold={args.geo_threshold}, sigmoid_temp={args.sigmoid_temperature}")
    ...
    for method in args.methods:
        ...
        if method == 'baseline':
            ...
            use_geo = args.geo_filter and not args.no_geo_filter
            prototta_model = setup_prototta(model, args.adaptation_mode, use_geo, 
                                            args.geo_threshold, args.importance_mode,
                                            args.sigmoid_temperature)
