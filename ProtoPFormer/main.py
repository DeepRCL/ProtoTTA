...
def main(args):
    ...
    # Auto-detect constrained /dev/shm (common on login nodes / containers).
    # PyTorch multiprocessing workers use /dev/shm for IPC; if it is tiny,
    # fall back to num_workers=0 which loads data in the main process.
    import shutil
    shm_free_mb = shutil.disk_usage('/dev/shm').free / (1024 ** 2)
    if shm_free_mb < 512 and args.num_workers > 0:
        logger.info(
            f"/dev/shm has only {shm_free_mb:.0f} MB free — "
            f"forcing num_workers=0 and pin_mem=False to avoid Bus error."
        )
        args.num_workers = 0
        args.pin_mem = False
