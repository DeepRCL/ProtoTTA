...
class SAR(nn.Module):
    ...
    def __init__(self, model, optimizer, steps=1, episodic=False, margin_e0=0.4*math.log(1000), reset_constant_em=0.2):
        ...
        self.adaptation_stats = {
            'total_samples': 0,
            'adapted_samples': 0,  # SAR only adapts filtered samples
    ...
    def forward(self, x):
        ...
        for _ in range(self.steps):
            outputs, ema, reset_flag, num_adapted = forward_and_adapt_sar(x, self.model, self.optimizer, self.margin_e0, self.reset_constant_em, self.ema)
            if reset_flag:
                self.reset()
            self.ema = ema  # update moving average value of loss
            
            # Track actual adaptation
            self.adaptation_stats['adapted_samples'] += num_adapted
            if num_adapted > 0:
                # One update per adapted sample per step
...
def forward_and_adapt_sar(x, model, optimizer, margin, reset_constant, ema):
    ...
    Returns: (model_output, ema, reset_flag, num_adapted_samples)
    """
    batch_size = x.size(0)
    optimizer.zero_grad()
    # forward
    ...
    return model_output, ema, reset_flag, num_filtered_2
